...
String calculateGPA(AsyncSnapshot<List<Course>> snapshot) {
  String
      semesterGradePointAverage; //SGPA [String] that is returned by the function

  //Loop Variable
  int productCount =
      0; // The loopVariable used to multiply the credits and grade achived in particular semester courses list of the user
  int creditsCount =
...
String calculateCGPA(AsyncSnapshot<List<Course>> snapshot) {
  ...
  int initialCredits = hiveGetData('manualCredits') == null
      ? 0
      : int.parse(hiveGetData('manualCredits'));
